...
async def upload_to_confluence(
    ...
    issue_details =  await fetch_issue_details.fetch_issue_details(request_body,confluence_config)
    html_content,page_title=  await process_download_attachment.process_attachments(request_body,issue_details,confluence_config)
    respone =  await confluence_publisher.create_or_update_confluence_page(confluence_config, request_body,page_title,html_content)
    return {respone}