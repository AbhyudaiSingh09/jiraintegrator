...
async def main(request_body: request_schemas.RequestBody):
 ...
 # Create a new instance of UpdatedRequestBody
    ...
    html_content,page_title=  await process_download_attachment.process_attachments(updated_request_body,issue_details,yaml_config)
    # respone =  await confluence_publisher.create_confluence_page(yaml_config, updated_request_body,page_title,html_content)
    respone =  await confluence_publisher.create_or_update_confluence_page(yaml_config, updated_request_body,page_title,html_content)
    return {respone}