...
async def download_attachment(attachment_url,request_body, filename,confluence_config):
    ...
    if response.status_code == 200:
        ...
        logger.info(f"Attachment file has been downloaded from Jira for issue {request_body.issue_key} and stored in {file_path}")
        return file_path
    else:
    ...
    if 'fields' in issue_details and 'attachment' in issue_details['fields']:
        ...
        for attachment in attachments:
            ...
            if attachment['filename'].endswith('.docx'):
                ...
                downloaded_file_path = await download_attachment(attachment_url,request_body,basefilepath,confluence_config)

                if downloaded_file_path:
                    page_title = await read_docx_file.get_page_title(downloaded_file_path)
                    logger.info(f"page_title has been extracted :{page_title}")
                    # Extract content from the downloaded .docx file
                    # Images
                    basefolder_path=  await get_images.extract_images_from_docx(downloaded_file_path,confluence_config)
                    # text
                    ...
                    html_content = await read_html_file.read_html_file(html_filename_path)
                    ...
                    await garbage_collector.remove_files_and_folder(basefilepath,confluence_config)
