...
async def replace_images_with_base64_in_html(html_file_path: str, images_folder: str, output_html_file_path: str):
    ...
    if not html_file_path or not images_folder or not output_html_file_path:
        logger.error("Invalid input parameters: NoneType found in html_file_path, images_folder, or output_html_file_path.")
        return
    ...
    for img_tag in image_tags:
        ...
        if not image_filename:
            logger.warning(f"Skipping image with empty filename found in {html_file_path}")
            continue
    ...
    try:
        async with aiofiles.open(output_html_file_path, 'w', encoding='utf-8') as output_file:
            await output_file.write(html_content)
        logger.info(f"Updated HTML content saved to {output_html_file_path}")
    except Exception as e:
        logger.error(f"Failed to write updated HTML content to {output_html_file_path}: {e}")