...
async def get_page_data(confluence_config, page_title) -> ConfluencePageResponse:
    ...
    response = requests.get(
        confluence_url, headers=headers, auth=confluence_config.api_token_v2
    )
    ...
    if response.status_code == 200:
        ...
        if response_data.get("results"):
            ...
            logger.info(
                f"Page data retrieved: ID = {page_data.id}, Title = {page_data.title}, Version = {page_data.version}"
            )
            return page_data
        else:
        ...
        logger.error(
            f"Failed to fetch page data: {response.status_code} - {response.text}"
        )
        return None
