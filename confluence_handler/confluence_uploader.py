...
async def update_confluence(
    request_body, html_content, confluence_config, page_data: ConfluencePageResponse
) -> str:
    confluence_url = confluence_config.confluence_uploader_url.format(
        confluence_page_id=page_data.id, domain=confluence_config.domain_identifier
    )
    ...
    payload = {
        ...
        "version": {"number": page_data.version, "message": confluence_config.message},
    }
